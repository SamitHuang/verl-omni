...
class TestFlowGRPOWindowScheduler:
    ...
    def test_none_size_propagates_none(self):
        ...
        assert ovr.to_sampling_overrides() == {}
...
class TestBuildSDEWindowScheduler:
    ...
    def test_mix_grpo_seed_renamed(self):
        """``seed`` (renamed from ``scheduler_seed``) drives the random draw."""
        cfg = DiffusionRolloutAlgoConfig(
            algo_type="mix_grpo",
            sample_strategy="random",
            sde_window_size=3,
...
class TestSDEWindowOverrides:
    def test_empty_overrides_to_dict(self):
        ovr = SDEWindowOverrides()
        assert ovr.to_sampling_overrides() == {}

    def test_full_overrides_to_dict(self):
        ovr = SDEWindowOverrides(sde_window_size=4, sde_window_range=[2, 6], extra={"k": 1})
        out = ovr.to_sampling_overrides()
        assert out == {"sde_window_size": 4, "sde_window_range": [2, 6], "k": 1}
