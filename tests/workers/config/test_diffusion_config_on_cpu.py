...
class TestDiffusionRolloutAlgoConfig:
    ...
    def test_algo_type_coerced_to_enum(self):
        cfg = DiffusionRolloutAlgoConfig(algo_type="flow_grpo")
        from verl_omni.workers.config.diffusion.rollout import AlgoType
        assert cfg.algo_type is AlgoType.FLOW_GRPO
    ...
    def test_to_rollout_dict_excludes_trainer_fields(self):
        cfg = DiffusionRolloutAlgoConfig(
            algo_type="mix_grpo", sde_window_size=4, sample_strategy="random", seed=42
        )
