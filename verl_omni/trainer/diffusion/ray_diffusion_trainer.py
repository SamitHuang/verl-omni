...
class RayFlowGRPOTrainer:
    ...
    def __init__(
        ...
        # (no per-step overrides) so legacy configs are unaffected.
    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # Per-step SDE-window overrides (MixGRPO sliding window / FlowGRPO static)
                window_overrides = self.sde_window_scheduler.get_window(self.global_steps)
                sampling_overrides = window_overrides.to_sampling_overrides()
                if sampling_overrides:
                    gen_batch.meta_info["algo_overrides"] = sampling_overrides
