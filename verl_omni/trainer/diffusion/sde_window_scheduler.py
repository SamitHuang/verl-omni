...
from verl_omni.workers.config.diffusion.rollout import AlgoType, DiffusionRolloutAlgoConfig, SampleStrategy
...
@dataclass
class SDEWindowOverrides:
    """Per-step SDE-window overrides injected into rollout sampling params."""

    sde_window_size: Optional[int] = None
    sde_window_range: Optional[list[int]] = None
    extra: dict[str, Any] = field(default_factory=dict)

    def to_sampling_overrides(self) -> dict[str, Any]:
        """Convert into a dict ready to merge with rollout sampling params."""
        out: dict[str, Any] = {}
        if self.sde_window_size is not None:
            out["sde_window_size"] = self.sde_window_size
        if self.sde_window_range is not None:
            out["sde_window_range"] = list(self.sde_window_range)
...
class SDEWindowScheduler(ABC):
    ...
    @abstractmethod
    def get_window(self, global_step: int) -> SDEWindowOverrides:
...
class FlowGRPOWindowScheduler(SDEWindowScheduler):
    ...
    def get_window(self, global_step: int) -> SDEWindowOverrides:  # noqa: ARG002
        return SDEWindowOverrides(
...
class _MixGRPOBase(SDEWindowScheduler):
    ...
    def _to_overrides(self, window_start: int) -> SDEWindowOverrides:
...
def build_sde_window_scheduler(
    ...
    *,
    ...
    if algo_config.algo_type == AlgoType.MIX_GRPO:
        ...
        if algo_config.sample_strategy == SampleStrategy.RANDOM:
            return MixGRPORandomScheduler(base_seed=int(algo_config.seed), **common)
        if algo_config.sample_strategy == SampleStrategy.PROGRESSIVE:
