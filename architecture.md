...
1. **Trainer (`RayFlowGRPOTrainer`)**
   Acts as the central "single-controller" running on the Ray driver process. It iterates over the dataset, orchestrates the sequence of rollout, reward computation, advantage estimation, and actor updates, while managing checkpoints and metrics logging.

2. **Agent Loop (`DiffusionSingleTurnAgentLoop`)**
   Extracts and formats multi-modal data from raw prompts. It wraps the batched queries into requests and dispatches them asynchronously to the rollout workers.

3. **Rollout Worker (`vLLMOmniHttpServer` / `vLLMOmniReplica`)**
   Acts as the generation backend by wrapping `vllm-omni`. It runs inference using SDE sampling to generate images (or latents) from prompts and computes the initial rollout log probabilities.

4. **Reward Worker (`VisualRewardManager`)**
   Evaluates the generated outputs (e.g., images) against user-defined metrics. It can dispatch images to Vision-Language Models (GenRM) or run rule-based heuristic functions (e.g., JPEG compressibility) to compute the RL reward signal.

5. **Actor Worker (`DiffusersFSDPEngine`)**
   Handles the heavy lifting of model training using PyTorch FSDP (Fully Sharded Data Parallel). It calculates Flow-GRPO policy loss using the estimated advantages and runs the distributed backward pass to update the diffusion model's weights.

6. **Pipelines / Schedulers (`DiffusionModelBase`)**
   Provides the architectural adapter layer bridging generic RL requests to specific HuggingFace diffusers implementations (like Qwen-Image) and handles the mathematical noise scheduling (e.g., FlowMatchSDE Flow Matching / Flow-Matching).
